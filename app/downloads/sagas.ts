...
} from '../services/services/download/interface';
 ...
 */
function* addDownloadItemSaga({
  downloadId,
  filePath,
  webContentsId
}: NewItemAction): SagaIterator {
...
function* initDefaultDownloadFolderSaga(): SagaIterator {
  const userDownloadFolder: string | undefined = yield select(
    getDownloadFolder
  );
  const downloadsPath = yield call(
    services.electronApp.getPath,
    ElectronAppPath.Downloads
  );
 ...
 * @returns
 */
function handleDownloadStateSaga(
  downloadId: string,
  downloadItemService: RPC.Node<DownloadItemService>
) {
  return function*({ state }: DownloadUpdatedStatePayload): SagaIterator {
    const savePath = yield call([
      downloadItemService,
      downloadItemService.getSavePath
    ]);
    yield put(updateItemPath(downloadId, savePath));
    if (state === 'progressing') {
      const receivedBytes = yield call([
        downloadItemService,
        downloadItemService.getReceivedBytes
      ]);
      const totalBytes = yield call([
        downloadItemService,
        downloadItemService.getTotalBytes
      ]);
 ...
 * @returns {SagaIterator}
 */
function* handleDownloadSaga(
  downloadItemService: RPC.Node<DownloadItemService>
): SagaIterator {
  const savePath: string = yield call([
    downloadItemService,
    downloadItemService.getSavePath
  ]);
  const downloadId: string = yield call([
    downloadItemService,
    downloadItemService.getDownloadId
  ]);
  const webContentsId: number = yield call([
    downloadItemService,
    downloadItemService.getWebContentsId
  ]);

  const itemStateChannel = serviceAddObserverChannel(
    downloadItemService,
    'onUpdated',
    'download-updated'
  );
  ...
  yield takeLatestWitness(
    itemStateChannel,
    handleDownloadStateSaga(downloadId, downloadItemService)
  );
  const doneState: DownloadDoneState = yield call([
    downloadItemService,
    downloadItemService.whenDone
  ]);
...
function* handleChangeDownloadFolderSaga(): SagaIterator {
  ...
  const chosenPath = yield callService(
    'download',
...
function* handleRevealPathInFinderSaga({
  path
}: revealPathInFinderAction): SagaIterator {
  if (path) {
...
export default function* main(): SagaIterator {
  const onWillDownloadChannel = serviceAddObserverChannel(
    services.download,
    'onWillDownload',
    'will-download'
  );
  ...
  yield all([
    ...
    takeEveryWitness(
      DOWNLOAD_FOLDER_BROWSE_CLICK,
      handleChangeDownloadFolderSaga
    ),
  ...
  services.electronApp.addObserver(
    observer({
      onPrepareQuit() {
